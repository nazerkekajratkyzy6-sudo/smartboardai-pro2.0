...
// createRoom шақырылғанда analytics да іске қосылады
const _origCreateRoom = window.createRoom;
window.createRoom = function() {
  _origCreateRoom();
  // Бөлме ID-ін алу үшін кішкентай delay
  setTimeout(() => {
    if (currentRoom) startAnalytics(currentRoom);
  }, 500);
};

// ── Бұрынғы room болса — analytics іске қосу ──────────
const savedRoom = localStorage.getItem("teacherRoomId");
if (savedRoom) {
  setTimeout(() => startAnalytics(savedRoom), 1000);
}
...
// AI Generator — PRO тексеру
const _origOpenAIGenerator = window.openAIGenerator;
window.openAIGenerator = function() {
  if (!requirePRO("ai_generator")) return;
  _origOpenAIGenerator && _origOpenAIGenerator();
};

// analyzePhoto — PRO тексеру
const _origAnalyzePhoto = window.analyzePhoto;
window.analyzePhoto = function() {
  if (!requirePRO("photo_analyze")) return;
  _origAnalyzePhoto && _origAnalyzePhoto();
};

// exportAnalytics — PRO тексеру
const _origExportAnalytics = window.exportAnalytics;
window.exportAnalytics = function() {
  if (!requirePRO("analytics_export")) return;
  _origExportAnalytics && _origExportAnalytics();
};

// generateAI — режим тексеру (жалпы chat FREE, қалғандары PRO)
const _origGenerateAI = window.generateAI;
window.generateAI = async function() {
  const action = document.getElementById("aiActionSelect")?.value || "chat";
  const freeActions = ["chat"]; // FREE-де тек chat
  if (!freeActions.includes(action) && !requirePRO("ai_center")) return;
  await _origGenerateAI();
};

// selectAIMode — PRO белгісін көрсету
const _origSelectAIMode = window.selectAIMode;
window.selectAIMode = function(btn, mode) {
  _origSelectAIMode && _origSelectAIMode(btn, mode);
  const freeActions = ["chat"];
  if (!freeActions.includes(mode) && currentPlan !== "pro") {
    const desc = document.getElementById("aiModeDesc");
    if (desc) {
      desc.innerHTML += ` <span style="
